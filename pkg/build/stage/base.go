...
func (s *BaseStage) getNextStageGitDependencies(_ Conveyor) (string, error) {
	...
	logboek.LogF("Stage %s next stage dependencies: %v\n", s.Name(), args, logboek.DebugLevel)
	sort.Strings(args)
	...
	for _, img := range images {
		for _, gitMapping := range s.gitMappings {
			...
			if commit != "" {
				...
				if !isOurAncestor {
					logboek.LogF("%s is not ancestor of %s for git repo %s: ignore image %s\n", commit, currentCommit, gitMapping.GitRepo().String(), img.ImageName, logboek.DebugLevel)
					continue ScanImages
				}
				logboek.LogF("%s is ancestor of %s for git repo %s: image %s is suitable for git archive stage\n", commit, currentCommit, gitMapping.GitRepo().String(), img.ImageName, logboek.DebugLevel)
			} else {
				logboek.LogF("WARNING: No git commit found in image %s, skipping\n", img.ImageName, logboek.DebugLevel)
				continue ScanImages
			}
		}
