...
func ImageLogProcessColorizeFunc(isArtifact bool) func(...interface{}) string {
	return imageDefaultFormat(isArtifact)
}
...
func imageDefaultFormat(isArtifact bool) func(...interface{}) string {
	var colorFormat []color.Attribute
	if isArtifact {
		colorFormat = []color.Attribute{color.FgCyan, color.Bold}
	} else {
		//colorFormat = []color.Attribute{color.FgMagenta, color.Bold}
